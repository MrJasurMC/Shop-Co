  ...
  return (
    <section className="px-4 md:p-10 py-6">
      ...
      <div className="mb-6 md:mb-[30px] flex justify-between border-b border-black/10">
        {["Product Details", "Rating & Reviews", "FAQs"].map((tab, i) => (
          <button
            key={tab}
            className={`flex-1 px-2 md:px-5 py-3 md:py-[14px] border-0 bg-transparent text-xs md:text-base cursor-pointer transition-all duration-150 text-center ${activeTab === i ? "font-semibold text-black border-b-2 border-b-black" : "text-[#777] border-b-2 border-b-transparent hover:border-b-black/30"}`}
      ...
      <div className="mb-8 md:mb-[50px]">
        <div className="flex items-center justify-between mb-5 md:mb-6">
          ...
          <button className="px-4 h-9 rounded-[62px] border border-black/20 text-sm font-medium hover:bg-black hover:text-white transition-all">Write a Review</button>
        </div>
        <div className="flex flex-col md:flex-row flex-wrap gap-4 md:gap-[20px]">
          {[
            { name: "Samantha D.", text: '"I absolutely love this t-shirt! The design is unique and the fabric feels so comfortable. As a fellow designer, I appreciate the attention to detail."', date: "August 14, 2023" },
            ...
            <div key={i} className="w-full md:w-[calc(50%-10px)] border border-black/10 rounded-2xl p-5 md:p-6 flex flex-col gap-2 md:gap-3 transition-all duration-200 hover:shadow-md">
              <div className="flex items-center justify-between">
                <img src={star} alt="stars" loading='lazy' />
                <img src={dots} alt="menu" loading='lazy' className="cursor-pointer hover:opacity-60 transition-opacity" />
              </div>
              <div className="flex items-center gap-[5px]">
                <h3 className="font-bold text-sm md:text-base">{review.name}</h3>
                <img src={check} alt="verified" loading='lazy' />
              </div>
              <p className="text-sm md:text-base text-[#333] leading-[22px]">{review.text}</p>
              <span className="text-black/60 text-xs md:text-sm">Posted on {review.date}</span>
            </div>
          ))}
        </div>
